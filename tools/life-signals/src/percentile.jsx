...
function PercentileTime({ daily, smoothing, birthYear, height=320 }) {
  ...
  const bands = daily.map(d => {
    ...
    return cdcBands(ageAt(ms, birthYear));
  });
        ...
        <g transform={`translate(${margin.left},${margin.top})`}>
          ...
          <g transform={`translate(${innerW - 220}, 10)`}>
            ...
            <g transform="translate(12, 32)">
              ...
              <text x="26" y="3" className="axis-text" style={{fontSize:10}}>US male median (age-adjusted)</text>
            </g>
...
function PercentileHistory({ daily, smoothing, birthYear, height=240 }) {
  const wrapRef = React.useRef(null);
  const w = useContainerWidth(wrapRef, 800);
  ...
  const pctLine = daily.map((d, i) => {
    ...
    const pct = cdcPercentile(ageAt(parseDay(d.d), birthYear), wv);
